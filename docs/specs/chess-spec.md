# Đặc tả API: Chess (Rooms & Game Spec)

Tài liệu này đặc tả chi tiết về cấu trúc dữ liệu, luồng hoạt động và các API/WebSocket Topic liên quan đến hệ thống
quản lý Phòng chơi (Lobby & Rooms) và Game cờ vua.

---

## 1. Tổng quan & Kiến trúc

Hệ thống sử dụng **Redis** làm database chính cho Room để tối ưu tốc độ và khả năng dọn dẹp rác (Garbage Collection)
khi người chơi ngắt kết nối. Các API được thiết kế theo dạng **Hybrid**:

- **REST API** để thực hiện các thao tác Command (Tạo phòng, Join phòng, Leave phòng) và lấy trạng thái khởi tạo (Initial State).
- **WebSocket (STOMP)** để truyền tải các sự kiện thay đổi trạng thái theo thời gian thực (di chuyển quân cờ, cập nhật ghế ngồi...).

---

## 2. Cấu trúc dữ liệu trong Redis

> **Kiến trúc Ponytail:** `host`, `white`, `black`, `spectators` trong Redis **chỉ lưu ID (chuỗi)**. Dữ liệu chi tiết (avatar, username) được Java Hydrate bằng 1 query DB duy nhất trước khi trả về Frontend — tránh rác JSON và race condition.
...
| `room:{roomId}:game:draw_offer`    | String            | ID người đề nghị hòa. TTL = `DRAW_OFFER_TTL_SECONDS` (mặc định 30s). Chỉ tồn tại khi có offer đang chờ. |
...
- **Mô tả:** Nếu có `query`, dùng Lua script (`search_lobby.lua`) để tìm kiếm case-insensitive trực tiếp trong Redis. Nếu không có `query`, dùng `ZREVRANGE` lấy trang hiện tại. Kết quả được Hydrate bằng pipeline (1 round-trip).
- **Response:** `{ content: RoomResponse[], page: { size, number, totalElements, totalPages } }`

**`RoomResponse`** (dùng cho lobby list — dữ liệu tối giản):
...
```json
{
  ...
  "status": "WAITING",
  "hostId": "1",
  "host": { "id": 1, "username": "player1", "avatarUrl": "..." },
  "white": { "id": 1, "username": "player1", "avatarUrl": "..." },
  "black": null,
  "createdAt": 1718029381000,
  "settings": { "timeMinutes": 3, "incrementSeconds": 2, "variant": "STANDARD", "rated": false, "isPrivate": false, "chatLocked": false, "spectatorLocked": false }
}
```
...
- **Response:** `RoomDetailResponse` (kèm `spectators[]`, `whiteReady`, `blackReady`, `gameData` nếu game đang diễn ra).
...
```json
{
  ...
  "white": { "id": 1, "username": "player1", "avatarUrl": "..." },
  "whiteReady": true,
  "black": { "id": 2, "username": "player2", "avatarUrl": "..." },
  "blackReady": true,
  "spectators": [],
  "createdAt": 1718029381000,
  "settings": { "timeMinutes": 3, "incrementSeconds": 2, "variant": "STANDARD", "rated": false, "isPrivate": false, "chatLocked": false, "spectatorLocked": false },
  "gameData": {
    ...
    "whiteRemainingMillis": 179000,
    "blackRemainingMillis": 180000,
    "turnStartedAt": 1718029390000,
    "moves": ["e2e4"]
  }
}
```

> `gameData` là `null` khi phòng ở trạng thái `WAITING` hoặc `COUNTDOWN`.
...
- **Response:** `RoomDetailResponse` (trạng thái phòng sau khi join).
...
- **Logic (Lua atomic `player_ready.lua`):**
    - Nếu cả hai người `isReady == true`, chuyển phòng sang `COUNTDOWN`, set `startAt` (hiện tại + 3s).
    - Nếu có người đổi sang `isReady == false` khi đang `COUNTDOWN`, chuyển phòng về `WAITING`, xoá `startAt`.
- **Side effects:**
    - Thành công (bất kể code nào): Broadcast `PLAYER_READY` tới `/topic/room/{roomId}`.
    - Khi bắt đầu đếm ngược: Lên lịch (Java `TaskScheduler`, key `start:{roomId}`) gọi `startGame` sau 3 giây. Broadcast `COUNTDOWN_STARTED` tới `/topic/room/{roomId}` và `ROOM_UPDATED` (status `COUNTDOWN`) tới `/topic/lobbies`.
    - Khi huỷ đếm ngược: Huỷ task `startGame`. Broadcast `COUNTDOWN_CANCELLED` tới `/topic/room/{roomId}` và `ROOM_UPDATED` (status `WAITING`) tới `/topic/lobbies`.
    - Khi task `startGame` chạy (qua `start_game.lua`): Khởi tạo hash `room:{roomId}:game`, chuyển phòng sang `IN_PROGRESS`. Broadcast `GAME_STARTED` tới `/topic/room/{roomId}` và `ROOM_UPDATED` (status `IN_PROGRESS`) tới `/topic/lobbies`. Đồng thời lên lịch turn timer cho white (key `turn:{roomId}`).
...
| `COUNTDOWN_STARTED`   | Khi cả 2 player sẵn sàng                       | `{ startAt, delayMillis }`                                                                                                          |
| `COUNTDOWN_CANCELLED` | Khi có người huỷ sẵn sàng trong lúc đếm ngược  | (No payload)                                                                                                                        |
| `GAME_STARTED`        | Khi đếm ngược kết thúc, ván đấu bắt đầu        | `{ whiteId, blackId, turn, fen, whiteRemainingMillis, blackRemainingMillis, turnStartedAt }`                                        |
| `MOVE_MADE`           | Sau mỗi nước đi hợp lệ                         | `{ move, color, newTurn, newFen, whiteRemainingMillis, blackRemainingMillis, turnStartedAt }`                                       |
| `DRAW_OFFERED`        | Một player đề nghị hòa                         | `{ offeredBy: "white"\|"black" }` — tự hết hạn sau `DRAW_OFFER_TTL_SECONDS` (mặc định 30s), không có event khi expire              |
| `DRAW_DECLINED`       | Đối thủ từ chối đề nghị hòa                    | (No payload)                                                                                                                        |
| `GAME_OVER`           | Khi ván đấu kết thúc (mọi nguyên nhân)         | `{ result, reason }`                                                                                                                |

#### `GAME_OVER` — Giá trị `result` và `reason`

| `result`                      | `reason`          | Mô tả                                        |
|:------------------------------|:------------------|:---------------------------------------------|
| `"WHITE_WIN"` / `"BLACK_WIN"` | `"CHECKMATE"`     | Chiếu hết                                    |
| `"WHITE_WIN"` / `"BLACK_WIN"` | `"TIMEOUT"`       | Đối thủ hết giờ (server-side turn timer)     |
| `"WHITE_WIN"` / `"BLACK_WIN"` | `"RESIGN"`        | Đối thủ đầu hàng                             |
| `"DRAW"`                      | `"STALEMATE"`     | Pat                                          |
| `"DRAW"`                      | `"DRAW_AGREEMENT"`| Hòa theo thỏa thuận (offer draw + accept)   |
...
- **Payload:** `{ "move": "e2e4" }` — UCI notation.
- **Guard:** Server kiểm tra lượt đi, tính hợp lệ của nước đi (qua `chesslib`), và thời gian còn lại.
...
### Send `/app/room.{roomId}.resign` — Đầu hàng

- **Auth:** Bắt buộc. Chỉ `white`/`black` mới được gọi (kiểm tra qua `isPlayerScript`).
- **Payload:** (không có)
- **Side effects:** Gọi `endGame(fromLoser(color), RESIGN)` → broadcast `GAME_OVER`.

### Send `/app/room.{roomId}.draw.offer` — Đề nghị hòa

- **Auth:** Bắt buộc. Chỉ `white`/`black`.
- **Payload:** (không có)
- **Guard:** Nếu đã có offer đang chờ (`SET NX` fail) → bỏ qua (spam guard).
- **Side effects:** Lưu `room:{roomId}:game:draw_offer = userId` với TTL, broadcast `DRAW_OFFERED`.

### Send `/app/room.{roomId}.draw.accept` — Chấp nhận hòa

- **Auth:** Bắt buộc.
- **Payload:** (không có)
- **Guard:** `GETDEL` — nếu không có offer, hoặc offer chính là của mình → bỏ qua.
- **Side effects:** Gọi `endGame(DRAW, DRAW_AGREEMENT)` → broadcast `GAME_OVER`.

### Send `/app/room.{roomId}.draw.decline` — Từ chối hòa

- **Auth:** Bắt buộc. Chỉ `white`/`black`.
- **Payload:** (không có)
- **Guard:** `GETDEL` — nếu không có offer, hoặc offer chính là của mình → bỏ qua.
- **Side effects:** Broadcast `DRAW_DECLINED`.

### Send `/app/room.{roomId}.chat` — Gửi tin nhắn chat

- **Payload:** `{ "message": "Nội dung chat" }`
- **Guard:** Nếu `settings.chatLocked == true`, server từ chối. Tin nhắn hợp lệ được lưu vào Redis (tối đa 10 tin) và broadcast `CHAT_MESSAGE`.
...
Giữ nguyên presence hash — chờ reconnect. Turn timer vẫn tiếp tục chạy phía server. Nếu player không reconnect và hết giờ, server tự broadcast `GAME_OVER` (reason `"TIMEOUT"`).
